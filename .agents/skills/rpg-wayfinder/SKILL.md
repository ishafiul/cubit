...
Implement against the approved issue and PRD using the repo's `/tdd` and `/code-review` workflows. For a large or multi-part task/subtask, make an ordered series of focused, coherent commits so reviewers can inspect each change independently; keep related tests and documentation with the change they support. Use a conventional commit message for each commit, and preserve the separate commits on the issue branch through review instead of squashing the whole implementation into one commit. A small, self-contained issue may use one focused commit. Push the issue branch, then create or update the PR. Format the PR title using Conventional Commits referencing the issue (e.g. `<type>(<scope>): <Summary> (#<issue-number>)`). The PR description must describe what was actually implemented and include verification results plus links to the issue, parent task, PRD, map, and relevant files or decisions. Use `Closes #<issue>` for a leaf subtask or task with no children. For a parent task with open subtasks, use `Part of #<task>` so its PR cannot close the rollup early.
