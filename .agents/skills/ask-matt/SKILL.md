...
- **A huge, foggy effort: a greenfield project or feature too big for one session** → Choose by destination. Use **`/wayfinder`** when the goal is a shared decision map and handoff after the route is clear. Use **`/rpg-wayfinder`** when the goal is a GitHub requirements intake, issue-based grilling, an approved RPG PRD, then capability tasks and feature subtasks. Use **`/grill-with-docs`** when the idea fits in one session.
