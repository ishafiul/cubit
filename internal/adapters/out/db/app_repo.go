...
func (r *AppRepo) Save(ctx context.Context, app *domain.Application) error {
	...
	query := `
		INSERT INTO applications (id, name, git_repo, branch, status, env_vars, bindings, active_deployment_id, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
	_, err := r.db.ExecContext(ctx, query,
		app.ID, app.Name, app.GitRepo, app.Branch, string(app.Status),
...
func (r *AppRepo) GetByID(ctx context.Context, id string) (*domain.Application, error) {
	query := `
		SELECT id, name, git_repo, branch, status, env_vars, bindings, active_deployment_id, created_at, updated_at
	...
	var (
		...
		statusStr, envJSON, bindingsJSON, createdAtStr, updatedAtStr string
		activeDepID                                    sql.NullString
	)

	err := row.Scan(
		&app.ID, &app.Name, &app.GitRepo, &app.Branch, &statusStr,
...
func (r *AppRepo) Update(ctx context.Context, app *domain.Application) error {
	...
	query := `
		UPDATE applications
		SET name = ?, git_repo = ?, branch = ?, status = ?, env_vars = ?, bindings = ?, active_deployment_id = ?, updated_at = ?
		WHERE id = ?
	`
	res, err := r.db.ExecContext(ctx, query,
		app.Name, app.GitRepo, app.Branch, string(app.Status),
		string(envVarsJSON), string(bindingsJSON), app.ActiveDeploymentID,
